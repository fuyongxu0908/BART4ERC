...
def evaluate(args, eval_loader, model, eval_or_test):
    ...
    def compute_metrics(preds_id, labels_id):
        ...
        if args.task_name in ['MELD', 'EmoryNLP', 'IEMOCAP']:
            macro_f1 = f1_score(labels_id, preds_id, labels=[i for i in range(7)], average='weighted')
            micro_f1 = f1_score(labels_id, preds_id, labels=[i for i in range(7)], average='micro')
        else:
            macro_f1 = f1_score(labels_id, preds_id, labels=[i for i in range(1, 7)], average='weighted')
            micro_f1 = f1_score(labels_id, preds_id, labels=[i for i in range(1, 7)], average='micro')
